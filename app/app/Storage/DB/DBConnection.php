...
class DBConnection
{
    ...
    private function connect(): PDO
    {
        ...
        try {
            $pdo = new PDO($dsn);
            if(empty($pdo)) {
                throw new Exception('DB connection is unsuccessful');
            }
            return $pdo;
    ...
    {
        if (empty($_ENV['db'])) {
            throw new Exception('DB section not declared in config file');
        }

        foreach (self::REQUIRED_VAR as $item) {
            if (!isset($_ENV['db'][$item])) {
                throw new Exception($item . ': is missing in config file');
            }
        }
    }
